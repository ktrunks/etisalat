...
class FinalizationApiCall {
    ...
    fun api(arg: Map<String, Objects>, result: MethodChannel.Result) {
        ...
        request.apiFinalizationCall(authData).enqueue(object : Callback<FinalizationResponse?> {
            override fun onResponse(
                    call: Call<FinalizationResponse?>,
                    response: Response<FinalizationResponse?>
            ) {
            ...
            override fun onFailure(call: Call<FinalizationResponse?>, t: Throwable) {
                val returnArg: HashMap<Int, String> = HashMap<Int, String>()
                returnArg[1] = "Error"
                returnArg[2] = "SomeThing went wrong, Please try again later."
                result.success(returnArg)
